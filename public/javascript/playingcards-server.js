...
DeckView = (function(_super) {
  ...
  DeckView.prototype.render2 = function() {
    var _this = this;
    this.$el.empty();

    var count = 0;
    for(i=0; i<Math.min(model.length, 5); i++){
      var cardView;
      cardView = new CardView({
        model: this.model[i],
        className: "card",
...
HandModel = (function(_super) {

  __extends(HandModel, _super);

  function HandModel() {
    return HandModel.__super__.constructor.apply(this, arguments);
  }

  HandModel.prototype.initialize = function(models, options) {
    var card, front, rank, suit, _i, _len, _results;
    front = options.front || false;
    cards = options.cards;
    console.log(cards);
    ...
    for (_i = 0, _len = cards.length; _i < _len; _i++) {
      _results[_i].front = front
    }
    return _results
  };

  HandModel.prototype.sort = function() {
    return this.models = _.sortBy(this.models, function(model) {
      var rank, rankOrder, suit, suitOrder;
      suit = model.get('suit');
      rank = model.get('rank');
  ...
  HandModel.prototype.shuffle = function() {
    return this.models = _.shuffle(this.models);
  };
